          ...
          <div className={ styles['grid-container'] }>
            ...
            <GridViewport positioningStyle='viewport-main' maxExtent={maxViewboxExtent}>
              <g transform={ zoomPanTransform(panCentre, zoomFactor) }>
                { [...Array(maxCourseExtent).keys()].map(idx => <line y1='0' y2={maxCourseExtent} x1={idx} x2={idx} style={{ stroke:'lightgray', strokeWidth:'0.2%' }} />) }
                { [...Array(maxCourseExtent).keys()].map(idx => <line x1='0' x2={maxCourseExtent} y1={idx} y2={idx} style={{ stroke:'lightgray', strokeWidth:'0.2%' }} />) }
                { [...Array(maxCourseExtent/10).keys()].map(idx => <line y1='0' y2={maxCourseExtent} x1={idx * 10} x2={idx * 10} style={{ stroke:'darkgray', strokeWidth:'0.2%' }} />) }
                { [...Array(maxCourseExtent/10).keys()].map(idx => <line x1='0' x2={maxCourseExtent} y1={idx * 10} y2={idx * 10} style={{ stroke:'darkgray', strokeWidth:'0.2%' }} />) }
            ...
            <GridViewport positioningStyle='viewport-left-axis' maxExtent={maxViewboxExtent}>
              <g transform={ panYTransform(panCentre.y, zoomFactor) }>
                { [...Array(maxCourseExtent/10 - 1).keys()].map(idx => {
                  const i = idx + 1;
                  const y = i * 10;
